...
router.post(
  ...
  async function (req, res) {
    try {
      ...
      // If the user has a previous image, delete it from Cloudinary
      if (user.image && user.image !== "default.png") {
        const publicId = user.image.split("/").pop().split(".")[0];
        await cloudinary.uploader.destroy(`uploads/${publicId}`);
      }

      // Update new Cloudinary image URL
...
router.post("/edit-pin/:id", upload.single("image"), async (req, res) => {
  try {
    ...
    if (req.file) {
      // Get the old post and delete the old image from Cloudinary
